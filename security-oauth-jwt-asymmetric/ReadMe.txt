...
Dependency : web, lombock, validation, h2, jpa, oauth2, configuration-processor
...
application.yml : Database Setup
...
Create a file UserInfoRepo in repo package, to create jpa-mapping using hibernate.
...
Let's create a package called userConfig and add few users to the database using CommandlineRunner
...
---------------------------------------------------------------------------------------------------
Manjunath@DESKTOP-UO92FND MINGW64 /e/pratibha/spring/IQ24/march/security-oauth-jwt-asymmetric/src/main/resources/certs (DEV)
...
Manjunath@DESKTOP-UO92FND MINGW64 /e/pratibha/spring/IQ24/march/security-oauth-jwt-asymmetric/src/main/resources/certs (DEV)
$ openssl rsa -in testkeypair.pem -pubout -out publicKey.pem
writing RSA key
...
Manjunath@DESKTOP-UO92FND MINGW64 /e/pratibha/spring/IQ24/march/security-oauth-jwt-asymmetric/src/main/resources/certs (DEV)
$ openssl rsa -in testkeypair.pem -pubout -out testpublicKey.pem
writing RSA key

Manjunath@DESKTOP-UO92FND MINGW64 /e/pratibha/spring/IQ24/march/security-oauth-jwt-asymmetric/src/main/resources/certs (DEV)
$ openssl pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in testkeypair.pem -out testprivateKey.pem
...
Let's now create a filterChain similar to /api as sign-in api, which will return accessToken, and also modify the Order
...
Create a AuthController , to receive the sign-in api
...
Now add the business logic to return the accessToken
...
Now add JwtTokenGenerator in jwtAuth package This builder is used to create a new JwtClaimsSet object, which represents the claims conveyed by a JSON Web Token (JWT).
...
Now modify /api config which will use ouath2 for authentication
...
Test the API for Authentication and Authorization:
...
Call the tokenFilter from the SecurityConfig
...
Let's understand difference between Access token and RefreshToken
...
RefreshToken must be saved in the database, to verify and return the access token: ** RefreshTokenEntity**
...
Create a refreshTokenGenerator method in JwtTokenGenerator. Remember, no API access scope should be added
...
Modify getJwtTokensAfterAuthentication so that when user sign-in, he will receive the refresh-token as well, so that when access token expires , it get's a new access token using refresh token.
...
We will be returning the refresh-token using HttpOnlyCookie so will need HttpServletResponseObject
...
Add the /refresh-token api , so that we can get new accessToken
...
Now, add the /refresh-token in securityConfig.
...
We can also create a custom filter to check the validity for refreshToken
...
Spring Security provide inbuilt api /logout to manage revoking.
...
Now test the api using refreshToken : http://localhost:8080/logout

Note: If you want to revoke from all the places, you can get the userName
...
Let's create a UserRegistrationDto
...
Create a endpoint in AuthController
...
Add respective service : AccessToken , RefreshToken in HttpOnly Cookie
...
Finally, add the endpoint for SecurityConfig
...
Testing with api : http://localhost:8080/sign-up
